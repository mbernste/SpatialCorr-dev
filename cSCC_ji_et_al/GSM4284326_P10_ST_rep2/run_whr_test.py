...
def main():
    ...
    meta_df = pd.read_csv('../GSM4284326_P10_ST_rep2_data/row_data.csv', index_col=0)
    meta_df['tissue'] = np.full(meta_df.shape[0], 1)

    # Load BayesSpace clusters
    df_bayes = pd.read_csv(f'../GSM4284326_P10_ST_rep2_data/BayesSpace_output.{n_clust}_clusts.tsv', sep='\t', index_col=0)
    ...
    df_dino = pd.read_csv('../GSM4284326_P10_ST_rep2_data/GSM4284326_P10_ST_rep2_stdata.filtered.dino.tsv.gz', sep='\t', index_col=0)
    ...
    df_counts = pd.read_csv('../GSM4284326_P10_ST_rep2_data/GSM4284326_P10_ST_rep2_stdata.filtered.tsv', sep='\t', index_col=0)
            ...
            for g1_i, g1 in enumerate(sorted(genes_meet_thresh)):
                for g2_i, g2 in enumerate(sorted(genes_meet_thresh)):
                    ...
                    if frozenset({g1, g2}) in cache_genes_to_pval.keys():
                        ...
                        pair_p_val, t_obs, t_nulls, kept_inds, obs_spot_lls, spotwise_t_nulls, pair_ct_to_p_val = spatialcorr.run_test(
                            adata,
                            [g1, g2],
                            sigma,
                            cond_key=cond_key,

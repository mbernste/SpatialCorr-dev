...
def main():
    ...
    parser.add_option("-s", "--sigma", help="Sigma (default: 5)")
    ...
    for gs_i, gs in enumerate(gene_sets):
        ...
        keep_ct_inds = [
            ind
            for ind, ct in enumerate(adata.obs['cluster'])
            if ct in keep_cts
        ]
        
        kernel_matrix_bhr = kernel_matrix[keep_ct_inds,:]
        kernel_matrix_bhr = kernel_matrix_bhr[:,keep_ct_inds]
        adata_bhr = adata[keep_ct_inds]

        ################ Run BHR on filtered dataset ################
        p_val, additional = spatialcorr.run_test(
            adata_bhr,
            genes_meet_thresh,
            sigma,
            cond_key='cluster',
            precomputed_kernel=kernel_matrix_bhr,
            contrib_thresh=10,
            verbose=1,
            n_procs=n_procs,
